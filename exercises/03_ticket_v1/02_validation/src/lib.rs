...
impl Ticket {
    ...
    fn new(title: String, description: String, status: String) -> Self {
        if title.trim().len() == 0 { panic!("Title cannot be empty"); }
        else if description.trim().len() == 0 { panic!("Description cannot be empty"); }
        else if title.len() > 50 { panic!("Title cannot be longer than 50 bytes"); }
        else if description.len() > 500 { panic!("Description cannot be longer than 500 bytes"); }
